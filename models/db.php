...
class db{
	...
	public $dbuser = 'vielmisa_admin';
	public $dbpass = 'Adm1n1strac10n';
	public $dbname = 'vielmisa_propietarios';
	public $db = null;
	...
	public function conectar(){
		if(($db = @mysqli_connect($this->dbhost, $this->dbuser, $this->dbpass)) === false){
			trigger_error('Error al conectarse a la base de datos.');
			exit;
		}
		 
		if((@mysqli_select_db($this->dbname, $db)) === false){
			trigger_error('Error al seleccionar la base de datos.');
			exit;
		}
		$this->db = $db;
	}
	...
	function select($campos, $tabla, $condicion=false, $order=false, $limit=false, $return=false){
		//Valido no usar * en la selección de campos
		/*if(eregi('\*', $campos)){
			die("No puede usar * en la selecci&oacute;n de campos");
		}*/
		
		//Valido que las partes del LIMIT sean números enteros.
